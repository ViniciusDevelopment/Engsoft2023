...
if (isset($_POST['deleteService'])) {
    ...
    if (Servico::DeletarServiço($Id)) {
        // Exclusão bem-sucedida, redirecione para a página atual sem o parâmetro "success"
        header('Location: page.php?arquivo=CadastrarServiço&id=');
        exit; // Certifique-se de sair do script após o redirecionamento
    } else {
        // Erro na exclusão, você pode exibir uma mensagem de erro aqui
            ...
            <tbody>
                ...
                if ($result->num_rows > 0) {
                    ...
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<div class='modal fade' id='modalExemplo1" . $row['Id'] . "' tabindex='-1' role='dialog' aria-labelledby='exampleModalLabel' aria-hidden='true'>";
                        echo "<div class='modal-dialog' role='document'>";
                        echo "<div class='modal-content'>";
                        echo "<div class='modal-header'>";
                        echo "<h5 class='modal-title' id='exampleModalLabel'>Confirmação</h5>";
                        echo "<button type='button' class='close' data-dismiss='modal' aria-label='Fechar'>";
                        echo "<span aria-hidden='true'>&times;</span>";
                        echo "</button>";
                        echo "</div>";
                        echo "<div class='modal-body'>";
                        echo "Tem certeza de que deseja deletar?";
                        echo "</div>";
                        echo "<div class='modal-footer'>";
                        echo "<button type='button' class='btn btn-secondary' data-dismiss='modal'>Não</button>";
                        echo "<form method='POST'>";
                        echo "<button type='submit' class='btn btn-danger' name='deleteService' value='" . $row['Id'] . "'>Sim</button>";
                        echo "</form>";
